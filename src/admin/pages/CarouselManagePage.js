...
import { DemoNotice, EmptyState, Field, LoadingState, Modal, PageHeading, PrimaryButton, SecondaryButton, StatusBadge } from '../components/AdminUI';
...
const CarouselManagePage = () => {
  ...
  useEffect(() => { adminApi.carousel().then((result) => setState({ loading: false, rows: result.data, isDemo: result.isDemo, warning: result.warning || '' })); }, []);
  ...
  return <div className="admin-page">
    ...
    {state.loading ? <section className="admin-panel"><LoadingState /></section> : state.rows.length ? <section className="admin-carousel-grid">
