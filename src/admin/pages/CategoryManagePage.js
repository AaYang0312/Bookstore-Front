...
import { DemoNotice, EmptyState, ErrorState, Field, LoadingState, Modal, PageHeading, PrimaryButton, SecondaryButton, StatusBadge, Table } from '../components/AdminUI';
...
const CategoryManagePage = () => {
  ...
  return <div className="admin-page">
    ...
    <section className="admin-panel">{state.loading ? <LoadingState /> : state.error ? <ErrorState message={state.error} /> : state.rows.length ? <Table columns={columns} rows={state.rows} /> : <EmptyState title="还没有分类" description="新建第一个图书分类。" />}</section>
