...
const BookManagePage = () => {
  ...
  const toggleStatus = async (book) => {
    try {
      ...
      setState((current) => ({ ...current, error: '', isDemo: current.isDemo || result.isDemo, rows: current.rows.map((row) => row.id === book.id ? { ...row, ...result.data } : row) }));
    } catch (error) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setState((current) => ({ ...current, error: '', isDemo: current.isDemo || result.isDemo, rows: payload.id ? current.rows.map((row) => row.id === payload.id ? result.data : row) : [result.data, ...current.rows] }));
      setEditing(null);
    } catch (error) {
      setState((current) => ({ ...current, error: error.message }));
    } finally {
  ...
  return (
    <div className="admin-page">
      ...
      <section className="admin-panel">{state.loading ? <LoadingState /> : state.error ? <ErrorState message={state.error} /> : state.rows.length ? <Table columns={columns} rows={state.rows} /> : <EmptyState title="没有找到图书" />}</section>
