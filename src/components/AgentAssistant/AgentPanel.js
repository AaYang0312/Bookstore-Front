...
const AgentPanel = ({ messages, isStreaming, onClose, onSend, onStop, onOpenBook, onAddToCart }) => {
  ...
  return (
    <section className="agent-panel" role="dialog" aria-label="智能购书助手">
      <header className="agent-panel-header">
        <div className="agent-title-wrap">
          <div className="agent-title-icon" aria-hidden="true">书</div>
          <div>
            <h2>智能购书助手</h2>
            <p><span className="agent-online-dot" />为你查找和比较好书</p>
          </div>
        </div>
        <button className="agent-icon-button" type="button" onClick={onClose} aria-label="关闭">×</button>
      </header>
      ...
      <div className="agent-message-list" ref={listRef} aria-live="polite">
        ...
        {isStreaming && messages[messages.length - 1]?.content === '' && (
          <div className="agent-message-row is-assistant">
            <div className="agent-avatar" aria-hidden="true">书</div>
